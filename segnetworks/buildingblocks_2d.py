...
class DistributionUncertainty(nn.Module):
    ...
    def forward(self, x, target):
        ...
        target_std = (target.var(dim=[2, 3], keepdim=False) + self.eps).sqrt()

        # sqrtvar_mu = self.sqrtvar(target_mean)
        # sqrtvar_std = self.sqrtvar(target_std)
...
class Encoder_DSU(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=(2, 2), pool_type='max', basic_module=DoubleConv, conv_layer_order='crg',
                 num_groups=8, pertubration=DistributionUncertainty, uncertainty= 0.5):
        super(Encoder_DSU, self).__init__()
        assert pool_type in ['max', 'avg']
        if apply_pooling:
            if pool_type == 'max':
                self.pooling = nn.MaxPool2d(kernel_size=pool_kernel_size)
    ...
    def forward(self, x, target, flag_ST):
        if self.pooling is not None:
            x = self.pooling(x)
            if flag_ST:
        ...
        if flag_ST:
            x = self.basic_module(x)
            target = self.basic_module(target)
            x = self.pertubration(x, target)
            # print('have style transfer')
        else:
            x = self.basic_module(x)
            target = []
        return x, target
