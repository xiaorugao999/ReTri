...
class weighted_bceloss_wdice(nn.Module):
    def __init__(self, hyperparameters):
        super(weighted_bceloss_wdice, self).__init__()
        self.class_num = hyperparameters['seg']['n_classes']
     
        self.softmax_logits =  nn.Softmax(dim = 1)
       
        #print('softmax_logits:{}'.format(softmax_logits.shape))
        loss_sum = []
        dice = 0 
    def forward(self, logits, targets):
        _, h ,w = targets.shape
        for i in range(self.class_num):
            target_tmp = torch.zeros_like(targets)
            target_tmp[targets == i] = 1
            inter = target_tmp.float() * self.softmax_logits(logits)[:,i,...]
            l = target_tmp.float() * target_tmp.float() 
            r = self.softmax_logits(logits)[:,i,...] * self.softmax_logits(logits)[:,i,...]
            dice += (2.0 * inter.sum()+1e-7)/(l.sum()+r.sum()+1e-7)
    
            amount_ratio = torch.sum(target_tmp.float()) / (h * w)
            tmp_ratio = 1 - amount_ratio
            tmp_loss = -1.0 * tmp_ratio * target_tmp.float() * torch.log(torch.clamp(self.softmax_logits(logits)[:,i,...],0.005,1))
            loss_sum.append( tmp_loss)
        loss_sum = torch.stack(loss_sum,1)
        # print('loss_sum:{}'.format(loss_sum.shape))
        wbce_loss = torch.mean(loss_sum)          
        dice_loss = -1.0 * dice / self.class_num
        # print('wce loss:{}, dice loss:{}'.format(wbce_loss,dice_loss))
    
        return wbce_loss + dice_loss 
...
def get_seg_loss_criterion(config,lossname):
    """
    Returns the loss function based on provided configuration
    :param config: (dict) a top level configuration object containing the 'loss' key
    :return: an instance of the loss function
    """
    assert 'seg' in config, 'Could not find loss function configuration'
    loss_config = config['seg']
    name = loss_config[lossname]
    ...
    elif name == 'weighted_bceloss_wdice':
         return weighted_bceloss_wdice(config)
