...
class SegAccessor (Dataset):
    def __init__(self, ds, labels, mask, xf, transforms, pipeline_type='cv', include_indices=False):
        ...
        if pipeline_type not in {'pil', 'cv'}:
            raise ValueError(
                'pipeline_type should be either \'pil\' or \'cv\', not {}'.format(pipeline_type))
    ...
    def __getitem__(self, sample_i):
        ...
        if self.mask_flag:
            if self.pipeline_type == 'pil':
                ...
                sample['mask_arr'] = np.full(
                    size_xy[::-1], 255, dtype=np.uint8)
                print('np.unique mask_arr: ...', np.unique(sample['mask_arr']))
            else:
                raise RuntimeError
        ...
        if self.transforms is not None:
            ...
            if 'sample0' in sample:
                print('SegAccessor after transforms np.unique mask: ...',
                      np.unique(sample['sample0']['mask']))
        return sample


def save_prediction(out_dir, img, label, pred_y, sample_name):
    path = os.path.join(out_dir, "{}.png".format(sample_name))
    os.makedirs(os.path.dirname(path), exist_ok=True)
    display_img = []
    display_img.append(img)
    display_img.append(mask2color_single(label))
    display_img.append(mask2color_single(pred_y))
    save_img = torch.cat(display_img, 3)
    torchvision.utils.save_image(save_img, path, nrow=1)

    # Image.fromarray(pred_y.astype(np.uint32)).save(path)


def mask2color_single(mask):

    maskarray = np.array(mask.cpu().squeeze()).astype(np.int)  # hw
    # print(maskarray.shape)
    color_dict = {0: [0, 0, 0], 1: [255, 255, 0], 2: [0, 255, 255], 3: [0, 255, 0],
                  4: [196, 58, 250], 5: [255, 0, 0]}
    mask2color = np.zeros(
        (maskarray.shape[0], maskarray.shape[1], 3)).astype(np.int)
    for k, v in color_dict.items():
        mask2color[maskarray == k] = v
    # maskarray = np.expand_dims(maskarray,-1).repeat(3,-1)
    masktensor = torch.from_numpy(mask2color)
    masktensor = masktensor.permute(2, 0, 1).unsqueeze(0)

    return masktensor


class DataSource (object):
    def save_prediction_by_index(self, out_dir, img, label, pred_y, sample_index):

        save_prediction(out_dir, img, label, pred_y,
                        self.sample_names[sample_index])

    def samplename(self, sample_index):
        return self.sample_names[sample_index]
...
class SegCollate (object):
        ...
        if padding is not None:
            ...
            if 'xf_pil' in sample:
                ...
                sample['xf_pil'] = affine.cat_nx2x3(
                    sample['xf_pil'][None, ...], affine.translation_matrices(np.array([[dx, dy]])))[0]
            if 'xf_cv' in sample:
                dy, dx = padding[1][0], padding[2][0]
                sample['xf_cv'] = affine.cat_nx2x3(affine.translation_matrices(
                    np.array([[dx, dy]])), sample['xf_cv'][None, ...])[0]
    ...
    def _compute_xf_0_to_1(pair):
        ...
        if 'xf_cv' in sample0 and 'xf_cv' in sample1:
            xf0_to_1_cv = affine.cat_nx2x3(
                sample1['xf_cv'][None, ...], affine.inv_nx2x3(sample0['xf_cv'][None, ...]))
            xf0_to_1 = affine.cv_to_torch(
                xf0_to_1_cv, sample1['image'].shape[1:3])
            pair['xf0_to_1_cv'] = xf0_to_1_cv[0]
            pair['xf0_to_1'] = xf0_to_1[0].astype(np.float32)
        elif 'xf_pil' in sample0 and 'xf_pil' in sample1:
            xf0_to_1_pil = affine.cat_nx2x3(affine.inv_nx2x3(
                sample0['xf_pil'][None, ...]), sample1['xf_pil'][None, ...])
            xf0_to_1 = affine.pil_to_torch(
                xf0_to_1_pil, sample1['image'].shape[1:3])
    ...
    def __call__(self, batch):
        ...
        if is_paired:
            # Paired
            for pair in batch:
                size = max(size[0], pair['sample0']['image'].shape[1]), max(
                    size[1], pair['sample0']['image'].shape[2])
                size = max(size[0], pair['sample1']['image'].shape[1]), max(
                    size[1], pair['sample1']['image'].shape[2])
        else:
            for sample in batch:
                size = max(size[0], sample['image'].shape[1]), max(
                    size[1], sample['image'].shape[2])
        ...
        if is_paired:
            for pair in batch:
                pair['sample0'] = SegCollate._pad_sample(
                    pair['sample0'], rounded_size)
                pair['sample1'] = SegCollate._pad_sample(
                    pair['sample1'], rounded_size)
            ...
            batch = [SegCollate._pad_sample(
                sample, rounded_size) for sample in batch]
