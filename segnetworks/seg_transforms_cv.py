...
from segnetworks import affine
from segnetworks.seg_transforms import SegTransform
...
class SegCVTransformPad (SegTransform):
    def pad_single(self, sample, min_size):
        ...
        if img_size[0] < min_size[0] or img_size[1] < min_size[1]:
            ...
            # Apply
            sample['image_arr'] = np.pad(
                image, [[h0, h1], [w0, w1], [0, 0]], mode='constant', constant_values=0)
            if 'labels_arr' in sample:
                sample['labels_arr'] = np.pad(sample['labels_arr'], [[h0, h1], [
                                              w0, w1]], mode='constant', constant_values=255)
            if 'mask_arr' in sample:
                sample['mask_arr'] = np.pad(
                    sample['mask_arr'], [[h0, h1], [w0, w1]], mode='constant')
            if 'xf_cv' in sample:
    ...
    def pad_pair(self, sample0, sample1, min_size):
        ...
        if img_size0[0] < min_size[0] or img_size0[1] < min_size[1]:
            ...
            # Apply
            sample0['image_arr'] = np.pad(
                image0, [[h0, h1], [w0, w1], [0, 0]], mode='constant', constant_values=0)
            sample1['image_arr'] = np.pad(
                image1, [[h0, h1], [w0, w1], [0, 0]], mode='constant', constant_values=0)
            if 'labels_arr' in sample0:
                sample0['labels_arr'] = np.pad(sample0['labels_arr'], [[h0, h1], [
                                               w0, w1]], mode='constant', constant_values=255)
                sample1['labels_arr'] = np.pad(sample1['labels_arr'], [[h0, h1], [
                                               w0, w1]], mode='constant', constant_values=255)
            if 'mask_arr' in sample0:
                sample0['mask_arr'] = np.pad(
                    sample0['mask_arr'], [[h0, h1], [w0, w1]], mode='constant')
                sample1['mask_arr'] = np.pad(
                    sample1['mask_arr'], [[h0, h1], [w0, w1]], mode='constant')
            if 'xf_cv' in sample0:
...
class SegCVTransformRandomCrop (SegCVTransformPad):
    ...
    def transform_single(self, sample):
        ...
        pos = np.round(extra * self.rng.uniform(0.0,
                       1.0, size=(2,))).astype(int)
        sample['image_arr'] = image[pos[0]:pos[0] +
                                    self.crop_size[0], pos[1]:pos[1]+self.crop_size[1]]
        if 'labels_arr' in sample:
    ...
    def transform_pair(self, sample0, sample1):
        ...
        pos0 = np.round(extra * self.rng.uniform(0.0,
                        1.0, size=(2,))).astype(int)
        pos1 = pos0 + np.round(self.crop_offset *
                               self.rng.uniform(-1.0, 1.0, size=(2,))).astype(int)
        ...
        sample0['image_arr'] = sample0['image_arr'][pos0[0]:pos0[0] +
                                                    self.crop_size[0], pos0[1]:pos0[1] + self.crop_size[1]]
        sample1['image_arr'] = sample1['image_arr'][pos1[0]:pos1[0] +
                                                    self.crop_size[0], pos1[1]:pos1[1] + self.crop_size[1]]
...
class SegCVTransformRandomCropScaleHung (SegCVTransformPad):
    ...
    def transform_single(self, sample0):
        ...
        sample0['image_arr'] = cv2.resize(
            image, self.crop_size[::-1], interpolation=cv2.INTER_LINEAR)

        if labels is not None:
            labels = labels[pos[0]:pos[0]+sc_size[0], pos[1]:pos[1]+sc_size[1]]
            sample0['labels_arr'] = cv2.resize(
                labels, self.crop_size[::-1], interpolation=cv2.INTER_NEAREST)

        if mask is not None:
            mask = mask[pos[0]:pos[0] + sc_size[0], pos[1]:pos[1] + sc_size[1]]
            sample0['mask_arr'] = cv2.resize(
                mask, self.crop_size[::-1], interpolation=cv2.INTER_LINEAR)
    ...
    def transform_pair(self, sample0, sample1):
        ...
        # Extract crop and scale to target size
        sample0['image_arr'] = sample0['image_arr'][pos0[0]:pos0[0] +
                                                    self.crop_size_arr[0], pos0[1]:pos0[1] + self.crop_size_arr[1]]
        ...
        sample1['image_arr'] = cv2.resize(
            sample1['image_arr'], self.crop_size[::-1], interpolation=cv2.INTER_LINEAR)
        ...
        if 'mask_arr' in sample0:
            ...
            sample1['mask_arr'] = cv2.resize(
                sample1['mask_arr'], self.crop_size[::-1], interpolation=cv2.INTER_NEAREST)
        ...
        if 'labels_arr' in sample0:
            ...
            sample1['labels_arr'] = cv2.resize(
                sample1['labels_arr'], self.crop_size[::-1], interpolation=cv2.INTER_NEAREST)

        if 'xf_cv' in sample0:
            xf01 = np.stack([sample0['xf_cv'], sample1['xf_cv']], axis=0)

            positions_xy = np.append(
                pos0[None, ::-1], pos1[None, ::-1], axis=0)
            # Matching `cv2.resize` requires:
            # - scale factor of out_size/in_size
            # - a translation of (scale_factor - 1) / 2
            scale_factors_xy = np.append(
                np.array([[1, 1]]),
                self.crop_size_arr[None, ::-
                                   1].astype(float) / sc_size1[None, ::-1],
                axis=0
            )
...
class SegCVTransformRandomCropRotateScale (SegCVTransformPad):
    ...
    def transform_single(self, sample0):
        ...
        if self.uniform_scale:
            ...
            scale_factor_yx = np.exp(
                self.rng.uniform(-self.log_max_scale, self.log_max_scale, size=(1,)))
            scale_factor_yx = np.repeat(scale_factor_yx, 2, axis=0)
        else:
            scale_factor_yx = np.exp(
                self.rng.uniform(-self.log_max_scale, self.log_max_scale, size=(2,)))
        rot_theta = self.rng.uniform(-self.rot_mag_rad,
                                     self.rot_mag_rad, size=(1,))
        ...
        # This we get the centre position of the scaled image in the coordinate system of the original image.
        centre = extra * \
            self.rng.uniform(0.0, 1.0, size=(2,)) + \
            np.minimum(sc_size, img_size) * 0.5

        # Build affine transformation matrix, the order: TRS(P)
        ...
        sample0['image_arr'] = cv2.warpAffine(
            image, local_xf[0], self.crop_size[::-1], flags=interpolation, borderValue=0, borderMode=cv2.BORDER_REFLECT_101)

        # Don't reflect labels and mask
        if 'labels_arr' in sample0:
            sample0['labels_arr'] = cv2.warpAffine(
                sample0['labels_arr'], local_xf[0], self.crop_size[::-1], flags=cv2.INTER_NEAREST, borderValue=255, borderMode=cv2.BORDER_CONSTANT)

        if 'mask_arr' in sample0:
            sample0['mask_arr'] = cv2.warpAffine(
                sample0['mask_arr'], local_xf[0], self.crop_size[::-1], flags=interpolation, borderValue=0, borderMode=cv2.BORDER_CONSTANT)

        if 'xf_cv' in sample0:
            sample0['xf_cv'] = affine.cat_nx2x3(
                local_xf, sample0['xf_cv'][None, ...])[0]
    ...
    def transform_pair(self, sample0, sample1):
        ...
        if self.constrain_rot_scale:
            if self.uniform_scale:
                scale_factors_yx = np.exp(
                    self.rng.uniform(-self.log_max_scale, self.log_max_scale, size=(1, 1)))
                scale_factors_yx = np.repeat(scale_factors_yx, 2, axis=1)
            else:
                scale_factors_yx = np.exp(
                    self.rng.uniform(-self.log_max_scale, self.log_max_scale, size=(1, 2)))
            ...
            if self.uniform_scale:
                scale_factors_yx = np.exp(
                    self.rng.uniform(-self.log_max_scale, self.log_max_scale, size=(2, 1)))
                scale_factors_yx = np.repeat(scale_factors_yx, 2, axis=1)
            else:
                scale_factors_yx = np.exp(
                    self.rng.uniform(-self.log_max_scale, self.log_max_scale, size=(2, 2)))
            rot_thetas = self.rng.uniform(-self.rot_mag_rad,
                                          self.rot_mag_rad, size=(2,))
        ...
        extra = np.maximum(img_size - sc_size, 0.0)
        centre0 = extra * \
            self.rng.uniform(0.0, 1.0, size=(2,)) + crop_centre_pos
        ...
        if 'xf_cv' in sample0:
            xf01 = affine.cat_nx2x3(local_xfs, np.stack(
                [sample0['xf_cv'], sample1['xf_cv']], axis=0))
            sample0['xf_cv'] = xf01[0]
            sample1['xf_cv'] = xf01[1]
...
class SegCVTransformRandomFlip (SegTransform):
    ...
    def transform_single(self, sample):
        ...
        # Flip flags, Flip or none flip
        flip_flags_xyd = self.rng.binomial(1, 0.5, size=(3,)) != 0
        flip_flags_xyd = flip_flags_xyd & np.array(
            [self.hflip, self.vflip, self.hvflip])

        sample['image_arr'] = self.flip_image(
            sample['image_arr'], flip_flags_xyd)

        if 'mask_arr' in sample:
            sample['mask_arr'] = self.flip_image(
                sample['mask_arr'], flip_flags_xyd)

        if 'labels_arr' in sample:
            sample['labels_arr'] = self.flip_image(
                sample['labels_arr'], flip_flags_xyd)

        if 'xf_cv' in sample:
            sample['xf_cv'] = affine.cat_nx2x3(
                affine.flip_xyd_matrices(
                    flip_flags_xyd, sample['image_arr'].shape[:2]),
                sample['xf_cv'][None, ...],
            )[0]
    ...
    def transform_pair(self, sample0, sample1):
        ...
        sample0['image_arr'] = self.flip_image(
            sample0['image_arr'], flip_flags_xyd[0])
        sample1['image_arr'] = self.flip_image(
            sample1['image_arr'], flip_flags_xyd[1])

        if 'mask_arr' in sample0:
            sample0['mask_arr'] = self.flip_image(
                sample0['mask_arr'], flip_flags_xyd[0])
            sample1['mask_arr'] = self.flip_image(
                sample1['mask_arr'], flip_flags_xyd[1])

        if 'labels_arr' in sample0:
            sample0['labels_arr'] = self.flip_image(
                sample0['labels_arr'], flip_flags_xyd[0])
            sample1['labels_arr'] = self.flip_image(
                sample1['labels_arr'], flip_flags_xyd[1])
        ...
        if 'xf_cv' in sample0:
            ...
            hv_flip_xf[flip_flags_xyd[:, 2]
                       ] = hv_flip_xf[flip_flags_xyd[:, 2], ::-1, :]
...
class SegCVTransformTVT (SegTransform):
    ...
    def transform_single(self, sample):
        if self.apply_single:
            sample = sample.copy()
            sample['image_arr'] = self._apply_to_image_array(
                sample['image_arr'])
    ...
    def transform_pair(self, sample0, sample1):
        if self.apply_pair0:
            sample0 = sample0.copy()
            sample0['image_arr'] = self._apply_to_image_array(
                sample0['image_arr'])

        if self.apply_pair1:
            sample1 = sample1.copy()
            sample1['image_arr'] = self._apply_to_image_array(
                sample1['image_arr'])
...
class SegCVTransformNormalizeToTensor(SegTransform):
    ...
    def transform_single(self, sample):
        ...
        # Convert image to float, divide the 255
        ...
        if image.shape[2] == 4:
            ...
            if self.mean is not None and self.std is not None:
                image = (
                    image - (self.mean[None, None, :] * alpha_channel)) / self.std[None, None, :]
        else:
            # Standardisation
            if self.mean is not None and self.std is not None:
        ...
        if 'mask_arr' in sample:
            sample['mask'] = img_as_float(sample['mask_arr'])[
    ...
    def transform_pair(self, sample0, sample1):
        ...
        if image0.shape[2] == 4:
            ...
            if self.mean is not None and self.std is not None:
                image0 = (
                    image0 - (self.mean[None, None, :] * alpha_channel0)) / self.std[None, None, :]
                image1 = (
                    image1 - (self.mean[None, None, :] * alpha_channel1)) / self.std[None, None, :]
        else:
            # Standardisation
            if self.mean is not None and self.std is not None:
                image0 = (
                    image0 - self.mean[None, None, :]) / self.std[None, None, :]
                image1 = (
                    image1 - self.mean[None, None, :]) / self.std[None, None, :]

        # Convert to NCHW tensors
        if image0.shape[2] != 3:
            raise ValueError(
                'image0 should have 3 channels, not {}'.format(image0.shape[2]))
        if image1.shape[2] != 3:
            raise ValueError(
                'image1 should have 3 channels, not {}'.format(image1.shape[2]))
        ...
        print('type img last transform:', type(sample0['image']))
        print('sample image.shape: ', sample0['image'].shape)
        print('sample image.shape: ', sample1['image'].shape)
        del sample0['image_arr']
        del sample1['image_arr']
        if 'mask_arr' in sample0:
            print('before img_as_float:', np.unique(sample0['mask_arr']))
            sample0['mask'] = img_as_float(sample0['mask_arr'])[
                None, ...].astype(np.float32)
            print('After img_as_float:', np.unique(sample0['mask']))
            print('type mask last transform:', type(sample0['mask']))
            sample1['mask'] = img_as_float(sample1['mask_arr'])[
                None, ...].astype(np.float32)
            del sample0['mask_arr']
            del sample1['mask_arr']

        if 'labels_arr' in sample0:
            sample0['labels'] = sample0['labels_arr'][None, ...].astype(
                np.int64)
            sample1['labels'] = sample1['labels_arr'][None, ...].astype(
                np.int64)
...
class SegCVTransformNormalizeToTensor_union(SegTransform):
    # for union train with multimodal translation network
    def __init__(self, mean, std):
        self.mean = None
        self.std = None
    ...
    def transform_single(self, sample):
        ...
        del sample['image_arr']
        if 'labels_arr' in sample:
            sample['labels'] = sample['labels_arr'][None,
    ...
    def transform_pair(self, sample0, sample1):
        ...
        # Convert image to float [0,1]
        image0 = img_as_float(sample0['image_arr'] * 255)
        image1 = img_as_float(sample1['image_arr'] * 255)
        ...
        sample0['image'] = image0.transpose(2, 0, 1).astype(np.float32)
        sample1['image'] = image1.transpose(2, 0, 1).astype(np.float32)
        sample0['image'] = sample0['image'][None, ...]
        sample1['image'] = sample1['image'][None, ...]
        print('type img last transform:', type(sample0['image']))
        print('sample image.shape: ', sample0['image'].shape)
        print('sample image.shape: ', sample1['image'].shape)
        del sample0['image_arr']
        ...
        if 'mask_arr' in sample0:
            print('before img_as_float:', np.unique(sample0['mask_arr']))
            sample0['mask'] = img_as_float(sample0['mask_arr'])[None,
                                                                None, ...].astype(np.float32)
            print('After img_as_float:', np.unique(sample0['mask']))
            print('type mask last transform:', type(sample0['mask']))
            sample1['mask'] = img_as_float(sample1['mask_arr'])[None,
                                                                None, ...].astype(np.float32)
            del sample0['mask_arr']
            del sample1['mask_arr']

        if 'labels_arr' in sample0:
            sample0['labels'] = sample0['labels_arr'][None, None, ...].astype(
                np.int64)
            sample1['labels'] = sample1['labels_arr'][None, None, ...].astype(
                np.int64)
            del sample0['labels_arr']
            del sample1['labels_arr']
