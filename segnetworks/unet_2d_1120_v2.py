...
class ResUNet2D_Featurelevel_cutmix(nn.Module):
    """
    Args:
        in_channels (int): number of input channels
        out_channels (int): number of output segmentation masks;
    ...
    def forward(self, img, img2, return_features = False, unary=None, if_self_attention = False, if_cross_attention = False, fea_b0_tea = None, **kwargs):
        x = img
        ...
        if if_cross_attention:
            ...
            x = rearrange(x, 'n c h w -> n (h w) c')
            fea_b0_tea = rearrange(fea_b0_tea, 'n c h w -> n (h w) c')
            x_cat = torch.cat([x, fea_b0_tea], dim=0)
