...
def dice_all_class_gpu(prediction, target, class_num=20):
    '''
    Calculate mean Dice coefficient for all classes (excluding background class 0)
    
    :param prediction: torch.Tensor with shape of [D, H, W], [H, W], or [H, W, D]
    :param target: torch.Tensor with shape of [D, H, W], [H, W], or [H, W, D]
    :param class_num: total number of classes including background
    :return: mean dice score across all foreground classes
    '''
    dices = []
    for i in range(1, class_num):
        # Check if class exists in either target or prediction
        target_has_class = torch.any(target == i)
        pred_has_class = torch.any(prediction == i)
        
        if not target_has_class and not pred_has_class:
            continue
            
        # print('class:', i)  # Uncomment for debugging
        target_per_class = torch.where(target == i, 1, 0)
        prediction_per_class = torch.where(prediction == i, 1, 0)
        dice = dice_per_class_gpu(prediction_per_class, target_per_class)
        dices.append(dice)
    
    if len(dices) == 0:
        # No valid classes found, return 0 dice
        return torch.tensor(0.0, device=prediction.device)
    
    # Stack dices and compute mean (preserve device and gradients)
...
def dice_per_class_gpu(prediction, target):
    ...
    :param prediction: torch.Tensor - binary mask for predicted class
    :param target: torch.Tensor - binary mask for target class
    :return: dice coefficient as torch.Tensor
    '''
    # Ensure eps is on the same device as input tensors
    eps = torch.tensor(1e-10, device=prediction.device)
    ...
    # print('sum pred + target', torch.sum(prediction) + torch.sum(target))
