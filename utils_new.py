...
def get_all_data_loaders(conf):
    ...
    if 'data_root' in conf:
        train_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'CT_train_2D'), batch_size, True,
                                                new_size_a, height, width, num_workers, False)
        test_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'CT_test_2D'), batch_size, False,
                                               new_size_a, new_size_a, new_size_a, num_workers, False)
        train_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'MRI_train_2D'), batch_size, True,
                                                new_size_b, height, width, num_workers, False)
        test_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'MRI_test_2D'), batch_size, False,
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=False):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5, 0.5, 0.5),
                                           (0.5, 0.5, 0.5))]
    # transform_list = [transforms.ToTensor()]
...
def __write_images(image_outputs, display_image_num, file_name, imgtype):
    ...
    # vutils.save_image(image_grid, file_name, nrow=1,imgtype = imgtype)
    save_image(image_grid, file_name, nrow=1, imgtype=imgtype)
...
def save_image(
    ...
    if imgtype == 'img':
        ...
        ndarr = grid.mul(255).clamp_(0, 255).permute(
            1, 2, 0).to('cpu', torch.uint8).numpy()
        # print('np.unique(ndarr)hhh',np.unique(ndarr[:,:,0]))

    print("type:", ndarr.dtype)
    # import cv2
...
# def write_2images(image_outputs, display_image_num, image_directory, postfix, imgtype):
#     n = len(image_outputs)
#     __write_images(image_outputs[0:n//2], display_image_num, '%s/gen_a2b_%s.png' % (image_directory, postfix),imgtype)
#     __write_images(image_outputs[n//2:n], display_image_num, '%s/gen_b2a_%s.png' % (image_directory, postfix),imgtype)

def write_2images_TTA(image_outputs, display_image_num, image_directory, postfixa, postfixb, imgtype):
    n = len(image_outputs) - 3
    __write_images(image_outputs[0:n//2], display_image_num,
                   '%s/gen_a2b_%s.png' % (image_directory, postfixa), imgtype)
    __write_images(image_outputs[n//2:n], display_image_num,
                   '%s/gen_b2a_%s.png' % (image_directory, postfixb), imgtype)


def write_2images(image_outputs, display_image_num, image_directory, postfixa, postfixb, imgtype):
    n = len(image_outputs)
    __write_images(image_outputs[0:n//2], display_image_num,
                   '%s/gen_a2b_%s.png' % (image_directory, postfixa), imgtype)
    __write_images(image_outputs[n//2:n], display_image_num,
...
def write_2images_single(image_outputs, display_image_num, image_directory, postfix, imgtype):
    __write_images(image_outputs, display_image_num,
                   '%s/%s.png' % (image_directory, postfix), imgtype)


def write_2images_supv(image_outputs, display_image_num, image_directory, postfix, imgtype):
    __write_images(image_outputs, display_image_num, '%s/seg_%s.png' %
                   (image_directory, postfix), imgtype)
...
def prepare_sub_folder(output_directory):
    ...
    best_checkpoint_directory_a = os.path.join(
        output_directory, 'a_best_checkpoints')
    best_checkpoint_directory_b = os.path.join(
        output_directory, 'b_best_checkpoints')
    if not os.path.exists(checkpoint_directory):
        print("Creating directory: {}".format(checkpoint_directory))
        os.makedirs(checkpoint_directory)
    if not os.path.exists(best_checkpoint_directory_a):
        print("Creating directory: {}".format(best_checkpoint_directory_a))
        os.makedirs(best_checkpoint_directory_a)
    if not os.path.exists(best_checkpoint_directory_b):
        print("Creating directory: {}".format(best_checkpoint_directory_b))
        os.makedirs(best_checkpoint_directory_b)
    return checkpoint_directory, image_directory, best_checkpoint_directory_a, best_checkpoint_directory_b
...
def get_test_model_list(dirname, key, numpt=-1):
    if os.path.exists(dirname) is False:
        return None
    gen_models = [os.path.join(dirname, f) for f in os.listdir(dirname) if
                  os.path.isfile(os.path.join(dirname, f)) and key in f and ".pt" in f]
    ...
    if gen_models is None:
        return None
    gen_models.sort()
    last_model_name = gen_models[numpt]
    return last_model_name
